...
def get_ai_reply(chat_id, user_id, user_name, user_text):
    ...
    if chat_id == config.OWNER_ID:
        triggers = ["kya hua", "kya chal raha", "update", "news", "kisi ne kuch kaha", "delete", "batao"]
...
async def toggle_chat(client, message):
    ...
    except: is_bot_admin = False

    if not is_bot_admin:
        await message.reply_text("❌ Admin banao pehle!")
        return
    if user_id != config.OWNER_ID:
        try:
            member = await client.get_chat_member(chat_id, user_id)
            if member.status not in [enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]: return 
        except: return
    ...
    if len(message.command) > 1:
        ...
        if action == "on":
            if chat_id not in enabled_chats:
                ...
                save_enabled_chats()
                await message.reply_text("Hello! Zoya is online! ❤️")
            else: await message.reply_text("Online hi hu!")
        elif action == "off":
            if chat_id in enabled_chats:
                enabled_chats.remove(chat_id)
                save_enabled_chats()
                await message.reply_text("Bye!,Mood off 😒")
...
async def ai_chat_handler(client, message):
    ...
    if msg_density > 5: reply_probability = 0.20
    if msg_density > 10: reply_probability = 0.40
    ...
    if is_private:
        ...
        is_active = (not is_bot_admin) or (chat_id in enabled_chats)
        if is_active:
    ...
    if response:
        # CLEAN ALL TAGS (Restored Full List)
        ...
        # Sticker Handling
        if "STICKER" in response or random.random() < 0.20:
